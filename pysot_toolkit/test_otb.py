...
# python -u test_otb.py --dataset OTB100 --name 'sot'
...
parser.add_argument('--name', default='sot', type=str,
...
def main():
    ...
    dataset_root = ''  # Set your OTB100 dataset path here
    net_path = 'checkpoint.pth.tar' #Absolute path of the model


    # create model
    net = NetWithBackbone(net_path=net_path, use_gpu=True)
    # 修改输入图像的尺寸
    tracker = Tracker(name='OTB100', net=net, window_penalty=0.49, exemplar_size=128, instance_size=288)
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if args.vis and idx > 0:
                    ...
                    try:
                        # 先尝试转换为浮点数，检查是否有 NaN
                        gt_bbox_float = list(map(float, gt_bbox))
                        if any(math.isnan(x) for x in gt_bbox_float):
                            print(f"Warning: NaN value detected in frame {idx}, skipping...")
                            continue  # 跳过这一帧
                        gt_bbox = list(map(int, gt_bbox_float))
                    except ValueError as e:
                        print(f"Error converting bbox values: {e}, values: {gt_bbox}")
                        continue  # 跳过这一帧
