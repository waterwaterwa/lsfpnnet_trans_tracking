...
"""
Feature Fusion Network for SOT tracker.
...
from .DYT_EASY import DyT


#导入自己的设定模块DYT

# 输入：[B, C, H, W] → 展平为 [H×W, B, C]
# 只使用 hidden_dim（256）作为通道维度
...
class FeatureFusionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
        ...
        decoderCFA_norm = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
...
class DecoderCFALayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.norm1 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm2 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
...
class FeatureFusionLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.norm11 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm12 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm13 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm21 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm22 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
        self.norm23 = DyT(normalized_shape=d_model, channels_last=True, alpha_init_value=0.5)
